...
import { editarPlatillo, obtenerAlimentoPorId } from '../../../Actions/AlimentoAction';
...
const EditarAComponent = () => {
    ...
    useEffect(() => {
        setIdAlimento(location.pathname.split('/')[2]);

        obtenerAlimentoPorId(idAlimento).then(response => {
            setNombre(response.data.nombre);
            setCantidad(response.data.cantidad);
            setDescripcion(response.data.descripcion);
            setTipoE(response.data.tipo);
        })
    }, [idAlimento]);
    ...
    const actualizarPlatillo = () => {
        ...
        if(nombre !== '' && precio !== '' && cantidad !== '' && tipo !== '') {
            editarPlatillo(idAlimento, alimento).then(response => {
        ...
        <div className="registrarProductorContainer">
          ...
          <form className={styles.formulario}>
            ...
            <Grid container spacing={3} justifyContent="center">
                <Grid item xs={12} md={6}>
                    <Button fullWidth variant="contained" size="large" onClick={actualizarPlatillo}>
                        Enviar
                    </Button>
                </Grid>
            </Grid>
...
export default EditarAComponent